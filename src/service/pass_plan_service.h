...
public:
    ...
    bool addPlan(const PassPlan& plan) {
        ...
        std::string sql = "INSERT INTO PASS_PLAN (plan_name,duration_days,price,description,is_active) VALUES ('" +
            escape(mysql, plan.plan_name) + "'," + std::to_string(plan.duration_days) + "," +
            std::to_string(plan.price) + ",'" + escape(mysql, plan.description) + "',1)";
    ...
    bool updatePlan(int id, const PassPlan& plan) {
        ...
        if (!conn) return false;
        MYSQL* mysql = conn->get();
        std::string sql = "UPDATE PASS_PLAN SET plan_name='" + escape(mysql, plan.plan_name) +
            "', duration_days=" + std::to_string(plan.duration_days) +
            ", price=" + std::to_string(plan.price) +
            ", description='" + escape(mysql, plan.description) +
    ...
    bool purchase(int userId, int planId, const std::string& licensePlate, std::string& error) {
        ...
        // Check if plate already has an active pass — extend instead of duplicate
        ...
        // Balance deduction is on a separate connection — refund explicitly on failure
        if (!tx.commit()) {
